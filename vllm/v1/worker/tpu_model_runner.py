...
class TPUModelRunner(LoRAModelRunnerMixin):
    ...
    def _prepare_inputs(self, scheduler_output: "SchedulerOutput"):
        ...
        logger.info(
            f"check slot_mapping_cpu type {type(self.input_batch.block_table.slot_mapping_cpu[:total_num_scheduled_tokens])}"
        )
        logger.info(
            f"check block_numbers * self.block_size type {type(block_numbers * self.block_size)}"
        )
        logger.info(f"check block_offsets type {type(block_offsets)}")
    ...
    def load_model(self) -> None:
        ...
        if self.lora_config is not None:
            assert not self.use_spmd, "LoRA is not supported with SPMD."
